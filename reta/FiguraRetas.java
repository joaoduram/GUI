...
public class FiguraRetas {
    public static void desenharReta(Graphics g, int x1, int y1, int x2, int y2, int esp) {
        RetaGr r = new RetaGr(x1, y1, x2, y2, esp);
        r.desenharReta(g);
    }
    ...
    public static void desenharRetas(Graphics g, int qtde, int esp) {
        ...
        for (int i = 0; i < qtde; i++) {
            ...
            // R, G e B aleatorio
            Color cor = new Color((int) (Math.random() * 256), (int) (Math.random() * 256),
                    (int) (Math.random() * 256));
